...
export const ProductCard = ({ img, description, name, dir }) => {
    return <Stack gap={'30px'} alignItems={'center'} justifyContent={'center'}>
        {/* box */}
...
export default function CardDetail({ theme, model, dir }) {
    ...
    return (
        <>
            <ProductCard img={import.meta.env.VITE_URL + product.attributes?.image?.data[0]?.attributes?.url} name={product.attributes?.name} description={products.attributes?.description} />
        </>
    )
}
